...
func createClusterMachine(name string) error {
	...
	actuator := aws.NewActuator(nil, nil, log.WithField("example", "create-machine"), "eu-west-1c")
...
func deleteClusterMachine(name string) error {
	_, machine := testClusterAPIResources(name)
	machine.Annotations = map[string]string{
		instanceIDAnnotation: name,
	}
	actuator := aws.NewActuator(nil, nil, log.WithField("example", "delete-machine"), "eu-west-1c")
...
func clusterMachineExists(name string) error {
	cluster, machine := testClusterAPIResources(name)
	machine.Annotations = map[string]string{
		instanceIDAnnotation: name,
	}
